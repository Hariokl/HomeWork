...
left = ["1 + 1 - 2 + 1", "2 - 1", "sin(90*)", "sin(-270*)", "10 / 2 * 10 // 25 - 1",
        "У Васи три яблока в правой руке и два в левой.\nУ него отняли два яблока из правой руки. "
        "В какой руке больше яблок?", "6! / 720", "30 + 1 / 1 - 1 - 31 + 2"]
...
unknown = ["У Васи три яблока в правой руке и два в левой.\n"
           "У него украли яблоки из сумки. Как зовут отца Пети?", "1 + 1 - 2 + 1 - 5", "tg(90*)",
           "100 // 10 - 5 + 16 - 27 - 10", "cos(90*) + 4"]
rooms = 10
room_n = 0
ab_room_n = 0
ab_room_v = 2, 10
...
while playing:
    ...
    if room_n == -1:
        ...
        while player_chose.lower() not in [str(i + 1) for i in
                                           range(len(choices) + (1 if len(cur_pos) > 1 else 0))] + [
            "аб", "лох", "лохов"] and \
    ...
    if room_n == 0:
        ...
        player_chose = input("\n>>>")
        while player_chose.lower() not in [str(i + 1) for i in
                                           range(len(choices) + (1 if len(cur_pos) > 1 else 0))] and \
                player_chose.lower() not in [x.split()[-1].lower() for x in choices]:
            player_chose = input(">>>")
...
Но помните: у вас не так много времени-60 секунд-за это время АБ успеет добежать до вас.""")
    ...
    if cur_pos[-1] != destination[len(cur_pos) - 1]:
        zad = unknown[randint(0, len(unknown) - 1)]
    elif destination[len(cur_pos)] == "1":
        zad = left[randint(0, len(left) - 1)]
    elif destination[len(cur_pos)] == "2":
        zad = right[randint(0, len(right) - 1)]
